...
function ObjednavkyB2B() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_URL}/get-tav-orders`, { params: Object.fromEntries(searchParams) });
  ...
  const handleRowClick = (orderId) => {
    ...
    navigate(`/objednavka-tav-detail/${orderId}`);
  };
